...
public class RegulationHistoryController {
	...
	public ResponseEntity<List<RegulationHistory>> getRegulationHistoryByRegulationId(@PathVariable Integer id) {
		List<RegulationHistory> reglist = reghistserv.getRegulationHistoryByRegulationId(id);
		return ResponseEntity.status(HttpStatus.OK).body(reglist);
	}

	@GetMapping("/export/history/{id}")
	public ResponseEntity<InputStreamResource> exportRegulationHistoryToExcel(HttpServletResponse response,
